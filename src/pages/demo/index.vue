...
/** 基本功能 */
const baseDemos = pagesJson.subPackages[0].pages
  .filter((e) => e.path.startsWith('demo/base') && !e.hide)
  .map((e) => ({
...
/** 页面功能 */
const pageDemos = pagesJson.subPackages[0].pages
  .filter((e) => e.path.startsWith('demo/page') && !e.hide)
  .map((e) => ({
    title: handleTitle(e.style?.navigationBarTitleText),
    path: e.path,
  }))
...
const routeInterceptionList = [
  {
    title: '说明页面',
    path: 'demo/route-interceptor/index',
  },
  {
    title: '单独登录页面',
    path: 'demo/route-interceptor/login-page?name=feige&age=30',
  },
  {
    title: '登录弹窗',
    path: 'demo/route-interceptor/login-model?name=feige&age=30',
  },
  {
    title: '静默登录',
    path: 'demo/route-interceptor/login-auto?name=feige&age=30',
  },
]
